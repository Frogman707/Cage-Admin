...
| **D1**　　| 스키마 실행 경로 · 마이그레이션 도구 | **`db/schema/`로 옮긴다. 마이그레이션 도구는 도입하지 않는다 — 빈 DB에 전체 재적용**　　　　| 2026-08-15 | ungkey |
...
## 12. D1 · 저장소 구조 — 실행 자산과 문서를 가른다

**결정**: `docs/`는 설계와 계획 문서만 담는다. 구현 · 서비스 · 배포에 쓰이는 파일은 `docs/` 밖에 둔다.

**결정의 입력**: `.sql` 13개가 `docs/architecture/ddl/`에만 있었다. 저장소에 실행 대상 경로가 없어 CI 잡도 계획 문서의 `Create:` 칸도 채울 수 없었다.

| 옮긴 것 | 어디로 |
|---|---|
| `docs/architecture/ddl/0*.sql` 13개 | **`db/schema/`** |
| 적용 절차 (문서에 셸 조각으로만 있던 것) | **`db/scripts/apply.sh` · `reset.sh`** |
| 골든 테스트 자리 | **`db/tests/`** — `helpers` · `fixtures` · `golden` · `posting` · `drift` |
| 문서 정합성 검사 | **`tools/check-doc-links.mjs`** (`R-12-10`) |

**남긴 것**: [`docs/architecture/ddl/README.md`](../architecture/ddl/README.md)는 제자리다. 파일 구성 · 적용 순서 예외 2건 · ENUM 변경 시 함께 고칠 곳 · 검증 이력을 담은 **설계 문서**이며 `R-01-50`(`SET CONSTRAINTS ALL IMMEDIATE` 금지 명문화) · `R-02-44`가 이 파일을 가리킨다.

**마이그레이션 도구는 도입하지 않는다.** U1=데모로 이관할 DB가 없다(§2). 운영 DB가 없으므로 증분 파일을 쌓지 않고 **번호 파일을 제자리에서 고친 뒤 빈 DB에 001~013 전체를 다시 적용**한다. 스펙도 그 전제로 쓰여 있다 — `R-01-22`는 "`004` 안에서 트리거 생성이 시드 INSERT 뒤에 온다"이지 `014`를 더하라가 아니다. sqitch · Flyway 판단은 운영 DB가 생기는 Track C로 미룬다.

**문서 표기 규약은 그대로다.** 문서가 `ddl/004`라고 부르는 것은 `db/schema/004_ledger.sql`이다. 번호가 이름이다.

**부수 조치**: `firebase.json`의 `hosting.public`이 `"."`이라 새 최상위 폴더는 기본적으로 공개 배포된다. `hosting.ignore`에 `db/**` · `tools/**` · `test/**` · `_bmad/**` · `_bmad-output/**`를 추가했다. **`db/**`를 지우면 스키마 전체가 공개된다.**

**검증**: 새 경로에서 PostgreSQL 18 컨테이너에 001~013 순차 적용 오류 0, `v_integrity_status` R1~R9 전부 `violations = 0`, `v_check_view_security` · `v_check_public_execute` 각 0행, 문서 상대 링크 1,955건 중 깨짐 0건.
...
| [`db/README.md`](../../db/README.md) | §12가 만든 실행 자산의 운영 문서 |
