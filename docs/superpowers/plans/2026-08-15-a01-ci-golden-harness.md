...
| `db/tests/reconciliation/integrity-status.test.js`    | `ledger.v_integrity_status` 9행 **이름** 대조 + R1~R7 `violations = 0` — R7이 `ledger.posting_rules` 표 대조다. R8·R9는 앵커 테이블이 비어 공허하다(아래 이월) |
...
| `R8_chain_anchor` · `R9_merkle_anchor` — 검사가 **구조적으로 공허하다**                                                    | 미정      | 두 뷰는 `audit.chain_anchors` · `audit.merkle_anchors`를 `FROM` 절에 둔다(`013:251` · `013:328`). 앵커를 쓰는 주체는 `audit_anchorer` 역할뿐이고(`012:249`) **그 역할을 쓰는 배치가 스키마·테스트·ROADMAP 어디에도 없다** — `db/tests/` 전체 grep 0건. 두 테이블이 항상 비어 뷰가 0행이므로 `count(*) FILTER (WHERE NOT ok)`는 무조건 0이다. 따라서 `v_integrity_status` 초록 아홉 행 중 **실질 커버리지는 일곱**이고, R8·R9는 "앵커 변조 없음"이 아니라 "앵커 없음"을 뜻한다. 앵커 기록 배치를 어느 계획이 맡을지가 먼저다 |
| `ledger.currencies.scale`이 **아무 데서도 읽히지 않는다**                                                                  | **a03**   | `001:278`에 컬럼과 `CHECK (scale BETWEEN 0 AND 6)`이 있으나 `002`~`013` 어디도 참조하지 않는다(실측 grep 0건). 금액이 전부 minor unit 정수라 DB 안에서는 당장 문제가 없지만, `KRW`(`scale = 0`)와 나머지(`scale = 2`)를 가르는 규약이 **선언만 돼 있고 강제되지 않는다**. `section-03-transfer.test.js:137` 주석이 같은 사실을 이미 적어 뒀다. 통화 계층을 손보는 a03이 읽는 쪽(입력 검증 · 표기 계약)을 정하거나 컬럼을 지운다 |
