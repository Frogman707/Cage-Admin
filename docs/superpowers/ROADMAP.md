...
|---|---|---|---|---|---|
| **a01** | [`a01-ci-golden-harness`](plans/2026-08-15-a01-ci-golden-harness.md) | [`12`](../spec/12-ci-golden-tests.md) 전부 | — | M0 | 🏁 구현 완료 — **이월 17건**, 그중 2건 주인 미정 (§3-2) |
...
### 3-2. 🏁 는 "그 계획이 끝났다"이지 "그 스펙이 다 됐다"가 아니다

계획이 `🏁 구현 완료`로 바뀌어도 그 계획의 **"이 계획의 범위 밖"** 표가 사라지지는 않는다. 이월은 다른 계획으로 **옮겨진 일**이지 없어진 일이 아니다. 상태 칸만 보고 그 스펙 범위를 닫힌 것으로 읽으면 안 된다.

**a01 — 이월 17건.** [계획의 "이 계획의 범위 밖"](plans/2026-08-15-a01-ci-golden-harness.md) 두 표에 전부 있다. 착수 시점 이월 9건 + 완료 후 브랜치 리뷰가 찾아낸 절 커버리지 빈칸 8건이다. 행선지:

| 이월 대상 | 건수 | 대표 항목 |
|---|---|---|
| a03 | 8 | `R-12-03` I1~I8 위반 · `R-12-10` · `R-12-14` · `R-01-24` 표 행 제거 · shortage 분기 · `p_to_wallet = false` · 통화 5종(`R-12-21`) · `currencies.scale` 소비처 |
| a06 | 4 | `04` §5 시작 유형 `account`·`marker` · §7·§8 정산 표 5~12행 · `R-12-04` 전 경로 4종 · 기간마감/실사 4개 연산 계약 |
| a10 · a13 · a14 | 1 | `04` §6-2 · §13 계열 분개 계약 (연산 함수가 아직 없다) |
| c04 · c06 · c08 | 1 | `R-12-22` KRW 금액 표기 |
| a02 | 1 | `R-12-20` 픽스처가 `provision_branch()`를 쓴다 — **완료** |
| **주인 미정** | **2** | **`R8_chain_anchor` · `R9_merkle_anchor`가 구조적으로 공허하다** · 통화 매트릭스 nightly 분리 |

**주인 미정 2건 중 앞의 하나가 자금 안전에 닿는다.** `audit.chain_anchors` · `audit.merkle_anchors`에 쓰는 주체는 `audit_anchorer` 역할뿐이고 그 역할을 쓰는 배치가 스키마·테스트·이 문서 어디에도 없다. 두 테이블이 항상 비어 뷰가 0행이므로 R8·R9의 `violations`는 무조건 0이다 — `v_integrity_status` 초록 아홉 행의 **실질 커버리지는 일곱**이고, 두 행은 "앵커 변조 없음"이 아니라 "앵커 없음"을 뜻한다. 앵커 기록 배치를 어느 계획이 맡을지가 M1 종료 조건보다 먼저 정해져야 한다.

a03 계획이 위 이월 중 3건(`04` §5 시작 유형 · §7·§8 정산 표 · `R-12-04` 4종)을 **a06으로 재배정**했다. 셋 다 `db/tests/fixtures/games.mjs`에 스텝업 인증·마커 발행 경로를 뚫는 일이고 a03의 스키마 변경과 무관하다. 사유는 [a03 계획의 "이 계획의 범위 밖"](plans/2026-08-16-a03-ledger-invariants.md)에 있다.
