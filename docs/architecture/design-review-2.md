...
> **진행 (2026-08-15) — 이 회차의 차단 4건이 해소됐다.** `DR-24`(뷰 13개 `security_invoker` + `integrity_ok()` 정의자화 + 드리프트 검사 뷰) · `DR-25`(`audit_reader` 신설) · `DR-26`(R8 앵커 대조 + `audit_anchorer` 분리 + 외부 서명 규약) · `DR-27`(트리거가 생성 열 식을 직접 평가 + 저장소 전수 확인). 반영 내역 전체는 [design-review.md](design-review.md) 머리말의 진행 표에 있다. **적용 검증은 아직이다** — 이 환경에 PostgreSQL이 없다 ([ddl/README.md](ddl/README.md)).

## 1. 요약

| ID | 항목 | 등급 | 영향 | 근거 |
|---|---|---|---|---|
| [DR-24](#dr-24) | 파생 뷰 12개가 정의자 뷰 — RLS 지점 격리 우회 | ~~차단~~ **해소** | M1 | `013` 전체 · `08-adr:480` |
| [DR-25](#dr-25) | `audit` 스키마를 읽을 수 있는 역할이 존재하지 않는다 | ~~차단~~ **해소** | M1 | `012:185` · `012:188` |
| [DR-26](#dr-26) | 앵커 대조 검사 부재 + 앱이 앵커를 쓴다 → 연쇄 위조 탐지 불가 | ~~차단~~ **해소** | M1 | `013:115` · `012:190` |
| [DR-27](#dr-27) | 실사 차액 강제가 생성 열 평가 시점 때문에 죽은 코드 | ~~차단~~ **해소** | M1 | `006:27` · `006:66` |
