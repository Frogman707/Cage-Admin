...
```
┌──────────── 브라우저 (모든 권한 보유) ────────────┐
│  인증      members/{id}.pw 를 받아와 JS 문자열 비교  │
│  잔액      원장 전량 다운로드 후 JS 합산             │
│  결과      Math.random()                            │
│  페이아웃  클라이언트가 배수 계산 후 원장에 기록      │
│  비밀값    index.html:6088 에 평문                   │
└───────────────────────┬─────────────────────────────┘
                        │   ⚠ 검증 계층 없음
                        ▼
┌──────── Firestore (보안 규칙 파일 없음) ────────────┐
│  members · accounts · memberLedger · partnerStaff   │
└─────────────────────────────────────────────────────┘
```

**서버 측 검증 지점 = 0개.** 브라우저를 신뢰하지 않으면 성립하는 기능이 하나도 없다.

즉시 폐기해야 하는 것:

| 항목 | 위치 | 문제 |
|---|---|---|
| 평문 비밀번호 + 클라이언트 비교 | `index.html:5663` · `partner-admin/app.js:186` | 로그인하려면 그 계정 비밀번호를 먼저 내려받아야 함 |
| 번들 하드코딩 시크릿 | `index.html:6088` | `view-source`로 즉시 획득. Git 히스토리에도 존재 |
| 자동 생성 마스터 계정 | `partner-admin/app.js:170-172` | `admin` / `0000` |
| 솔트 없는 SHA-256 마스터 비밀번호 | `index.html:6307` | 레인보우 테이블 대상, 상수가 번들에 존재 |
| 클라이언트 통과 TOTP 시크릿 | `index.html:4300-4305` | 비밀키가 브라우저를 지나감 |
| `Math.random()` RNG | `shared/game-engine.js` | 게임 결과 생성 |
| 저장형 XSS | `partner-admin/app.js:585-595` | 회원 닉네임·ID를 `innerHTML`·`onclick`에 직접 삽입 |

> **시크릿 회전이 먼저다.** 코드에서 지우는 것만으로는 무효화되지 않는다. Git 히스토리에 남아 있다.
...
현행 구현(`index.html:5511-5576`)을 **서버로 이전**해 재사용한다. 알고리즘은 검증된 것을 그대로 쓰되 시크릿이 브라우저를 지나가지 않게 한다.
...
| 필드 | 현행 상태 | 신규 대응 |
|---|---|---|
| `staff.pin` | 평문 (`index.html:5663` — 데모 전원 `'1234'`) | Argon2id 해시 |
| `staff.totpSecret` | 평문, 브라우저를 통과 (`:4300-4305`) | KMS 봉투 암호화 |
...
| 위협 | 대응 |
|---|---|
| 저장형 XSS | 출력 이스케이프 전수 적용. `onclick` 문자열 삽입 폐기 → `data-*` + 이벤트 위임. CSP 헤더 |
...
현행 `telegramWebhook`은 `/start SE7419`를 보낸 **누구든** 그 계좌에 자기 chat을 연결한다 (`functions/index.js:63-78`). 계좌 ID가 짧고 규칙적이라 열거가 쉽다. 출금 인증 메시지가 이 채널로 가므로 계좌 탈취로 이어진다.

**신규:**
1. 계좌 ID를 직접 받지 않는다. **로그인 세션에서 발급한 1회용 단기 토큰**만 페이로드로 허용
2. 토큰은 사용 즉시 소멸, 수명 5분
3. chat 단위 rate limit
4. 문서 ID 조립 전 형식 검증
...
구현 착수와 무관하게 **지금 당장** 해야 하는 것:

- [ ] `TG_APP_API_SECRET` 회전 (Git 히스토리에 존재)
- [ ] 배포된 Firestore 보안 규칙 실제 상태 확인 (저장소에 파일이 없어 콘솔 확인 필요)
- [ ] `partner-admin/app.js` 렌더링 경로 이스케이프 적용
- [ ] `admin` / `0000` 자동 생성 마스터 계정 제거
- [ ] Telegram 연동 1회용 토큰 도입
