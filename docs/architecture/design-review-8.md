...
**대조 기준:** [`00`](00-system-map.md) §4-3·§5·§8 · [`02`](02-target-architecture.md) §4-2 · [`04`](04-posting-rules.md) §13 · [`ddl/001`](../../db/schema/001_types_and_extensions.sql)·[`004`](../../db/schema/004_ledger.sql)·[`005`](../../db/schema/005_games_rolling.sql)·[`008`](../../db/schema/008_post_transaction.sql) · [`docs/avatar-speed/`](../avatar-speed/README.md) 8종
**상태:** 미해결 5건 — 차단 0 · 높음 2 · 중간 2 · 낮음 1 · 반증 7건

> **후속**: [design-review-9.md](design-review-9.md) — 9차 `DR-83`~`DR-86` (차단 0 · 높음 2 · 중간 2). 케이지 기준선 정정(6차 §6-1) 작업에서 파생됐다. 이 문서의 `DR-78`(멱등키 충돌)·`DR-81`(배당 권위 부재)과 각각 짝을 이루는 항목이 있다 — `DR-85`(멱등키 부재)·`DR-84`(커미션 요율 권위 부재). 아홉 문서 합계 **86건 · 차단 13 — 2026-08-15 전부 해소** (`DR-38`만 부분 해소)
...
| `DR-79` | 높음 | 거래 행위자 모델에 플레이어가 없다 | [`ddl/004`](../../db/schema/004_ledger.sql):69·90-91 · [`ddl/001`](../../db/schema/001_types_and_extensions.sql):123-129 |
| `DR-80` | 중간 | 실시간 채널 8종이 전부 케이지 어드민 것이다 | [`02`](02-target-architecture.md) §4-2 (:116-127) |
| `DR-81` | 중간 | 배당률·커미션의 권위 소재가 설계 어디에도 없다 | [`04`](04-posting-rules.md) §13 · [`ddl/005`](../../db/schema/005_games_rolling.sql) |
...
| 근거 | 위치 |
| --- | --- |
| `ledger.idempotency_keys.key TEXT PRIMARY KEY` — 연산 이름 스코프 없음 | [`ddl/004`](../../db/schema/004_ledger.sql):501-502 |
| `transactions.idempotency_key TEXT NOT NULL UNIQUE` | [`ddl/004`](../../db/schema/004_ledger.sql):64 |
| 같은 키 + 다른 페이로드 → `422 idempotency-key-reused` 예외 | [`ddl/008`](../../db/schema/008_post_transaction.sql):176-179 |
| 키 없이 호출 → `idempotency key is required` 예외 | [`ddl/008`](../../db/schema/008_post_transaction.sql):144-146 |
...
[`ddl/004`](../../db/schema/004_ledger.sql):61의 `ledger.transactions`가 행위자를 이렇게 잡는다:
...
행위자는 **직원뿐**이다. `auth_method` ENUM([`ddl/001`](../../db/schema/001_types_and_extensions.sql):123-129)도 `pin` · `totp` · `withdraw_pw` · `approval` · `system` 다섯 값이고, 마지막 값의 주석은 *"배치 · 마이그레이션"* 이다. 회원이 자기 이름으로 인증했다는 것을 표현할 값이 없다.
...
**필요 조치.** ① `auth_method`에 회원 인증 값 추가 ② 행위자를 `actor_party_id`로 일반화하거나 `on_behalf_of_party_id`를 더해 2행 모델로 ③ `tx_actor_required` CHECK 재작성 ④ [`ddl/004`](../../db/schema/004_ledger.sql):551의 감사 조회 뷰가 `identity.staff`만 LEFT JOIN하므로 함께 수정.
...
목표 설계에는 대응물이 없다 — [`ddl/005_games_rolling.sql`](../../db/schema/005_games_rolling.sql)에서 `배당`·`payout`·`odds`·`commission` 검색 결과 **0건**. 게임별 배당표를 담을 테이블도, 값을 검증할 제약도 없다.
...
**미검증 잔여.** [`ddl/005`](../../db/schema/005_games_rolling.sql)·[`010`](../../db/schema/010_operations_game.sql)의 케이지 게임·롤링 계층은 이번 범위 밖이다 (3차에서 op 함수 전수 조사는 했으나 `01` §7~§11의 케이지 게임 흐름과의 대조는 미수행). [`05`](05-api-contract.md) 줄 참조 재검증도 이월.
