...
현행의 가장 큰 약점은 9개 `shift` 카운터가 서로 독립적으로 누적되며 **상호 정합성을 검증할 수단이 없다**는 것이다 (`index.html:4828`, [01번 문서 9절](01-current-system.md)).
...
**정정은 삭제가 아니라 역분개다.** 원 거래의 모든 분개를 부호 반전해 새 거래로 기록하고 `reverses_tx_id`로 연결한다. 현행 게임 취소는 문서를 삭제하지만(`index.html:4529-4537`), 신규 모델에서는 흔적이 남는다.
...
현행 `cageConfig`의 `*Baseline` 스칼라 6종(`index.html:4886`)을 기간 엔티티로 대체한다.
