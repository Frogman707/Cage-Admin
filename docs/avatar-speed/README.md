...
상수 8종, 함수 26개 전체. 인자·반환 구조·Firestore 쓰기 내용·엣지 케이스. 순수 함수 17개를
표로 구분해 두어 테스트 대상을 바로 고를 수 있습니다. 푼토 반코 타블로 전체 표 포함.

**[`avatar/app.js` 앱 레퍼런스](reference-avatar-app.md)**
전역 상태, 뷰 5개, 부팅·인증 흐름, 아바타 모드 전체, 스피드 모드 전체, DOM ID 계약,
로드맵 렌더링 창 크기, 2026-08 UI 개편으로 사라진 요소 목록.
...
서드카드 룰(푼토 반코 타블로)까지 구현된 바카라. 덱 모델, 카드값, 드로우 판정표, 배당 계산,
페어 판정, 로드맵 4종 알고리즘, 실제 바카라와의 차이 정리표.

**[알려진 격차 (G-01 ~ G-13)](explanation-known-gaps.md)**
코드 정독으로 확인한 결함 13건. 각 항목마다 근거 파일·줄, 결과, 고치는 법, 재현 절차.
의도된 단순화와 구분해 두었습니다.
...
| 동시 진행 테이블 | 1개 | 로비에 열린 전부 |
| 베팅 조작 위치 | 없음 (자동) | **연 테이블 안에서만** (목록에는 스팟 없음) |
...
목록 카드도 두 모드가 같은 `.lobby-card` 마크업을 씁니다 — 스피드만 상태 배지 자리에
카운트다운이 들어갑니다.
...
| 파일 | 역할 |
| --- | --- |
| [`avatar/index.html`](../../avatar/index.html) | 화면 뼈대, 로그인 게이트, 모달, 스크립트 로드 순서 (213줄) |
| [`avatar/app.js`](../../avatar/app.js) | 두 모드의 상태·뷰·라운드 루프 (1,175줄) |
| [`shared/game-engine.js`](../../shared/game-engine.js) | 결과 생성(타블로), 인증, 원장 쓰기, 로드맵 (346줄) |
| [`shared/cage-ui.js`](../../shared/cage-ui.js) | Firebase 부트스트랩, 원장 이중 쓰기, 포맷 유틸 (211줄) |
| [`shared/i18n.js`](../../shared/i18n.js) | ko / zh / en / ja / vi 5개 언어 (205줄) |
| [`shared/game-ui.css`](../../shared/game-ui.css) | 칩·카드·로드맵 CSS 클래스 계약 (550줄) |
| [`shared/assets/table-live.jpg`](../../shared/assets/) | 테이블 스테이지·로비 썸네일 라이브 스틸 |
...
- **작성 기준일**: 2026-08-15 · 브랜치 `backend` (최초 작성 2026-08-12, 코드 변경 반영해 갱신)
...
최초 작성 이후 `avatar/`·`shared/`에 커밋 17개가 들어갔습니다. 반영한 변경:

| 변경 | 커밋 | 영향받은 문서 |
| --- | --- | --- |
| **서드카드 룰 구현** — `dealHand()` 제거, 푼토 반코 타블로 전체 | `9eec17d` | 룰과 로드맵, 엔진 레퍼런스, 튜토리얼, 알려진 격차 |
| **파생 로드 연속 규칙 수정** — `L > j ? red` → `L === j ? blue` | `9eec17d` | 룰과 로드맵, 엔진 레퍼런스 |
| **비드 플레이트 규칙 변경** — 시간순 6행 채움 → 빅로드식 열 그룹핑, `BEAD_WINDOW` 40 도입 | `6a82b3d` | 룰과 로드맵, 엔진 레퍼런스, 앱 레퍼런스 |
| **스코어보드 재배치** — 3밴드 → 빅로드 + 파생 3종 4밴드 | `0f16091`, `6a82b3d` | 룰과 로드맵, 앱 레퍼런스 |
| **스피드 타일 베팅 제거** — 베팅은 상세 화면에서만 | `086447f` | 앱 레퍼런스, 처리 흐름, 튜토리얼, README |
| **스피드 목록을 아바타 카드로 통일** — 미니 스테이지·단계 캡션·하단 칩 트레이 삭제 | `c10fd3a`, `b54e945` | 앱 레퍼런스, 튜토리얼 |
| **아바타 신청 CTA 배지 삭제** — 빈 테이블 카드에 배지 없음 | `40cc6cf` | 앱 레퍼런스, 하우투, 튜토리얼 |
| **하이퍼/드래곤타이거 게임 타입 삭제** | `8a2246d` | 앱 레퍼런스, 알려진 격차 |
| **라이브 스틸 이미지 도입** | `e85a8e3`, `f69a60c` | 앱 레퍼런스, 알려진 격차 |
| **G-13 신규** — 스피드 상세의 팁 버튼이 토스트만 띄움 | — | 알려진 격차, 앱 레퍼런스, 하우투 |

G-01 ~ G-12는 모두 그대로 유효하며, 줄 번호 참조를 현행 코드에 맞춰 갱신했습니다.
