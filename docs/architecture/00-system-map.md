...
| 회원 보유금 원장 | 3개 앱 공통 | `01` §13 (12줄) | `04` §13 (베팅·페이아웃만) | ⚠ ENUM만 |
| 계좌 마스터 데이터 | **`localStorage`** | ❌ 없음 | `ledger.parties` | ⚠ 이관 경로 없음 |
| **플레이어 라운드 · 베팅** | `avatar/` + `game-engine.js` | ✅ `avatar-speed/` | ❌ **없음** | ❌ 없음 |
| **아바타 대리베팅** | `avatar/` + `partner-admin/` | ✅ 양쪽 | ❌ **없음** | ❌ 없음 |
| **파트너 콘솔 58화면** | `partner-admin/` | ✅ `partner-admin/` | ❌ **없음** | ❌ 없음 |
| **포인트 · 파트너 쉐어** | `partner-admin/` | ⚠ 화면 목록만 | ❌ **없음** | ❌ 없음 |
...
| 접두사 | 출처 | 개수 | 대상 |
| --- | --- | --- | --- |
| `S1`·`S2`·`S4`·`S7`·`S8` · `C1`~`C5` · `Q1`~`Q4` | Track A 하드닝 작업 | 13 | 저장소 전체 |
| `P2` | Track A 성능 항목 | 1 | 실시간 구독 |
...
**`P2`(성능 항목)와 `P-02`(빈 입력 로그인이 `admin`이 됨)가 서로 다른 것을 가리킨다.** 하이픈
하나 차이다. 통합 시 접두사를 재배정해야 한다 (§8 개선 항목 A6).
...
| **A5** | 파트너 주체 모델 | `partners`·`partnerStaff`가 `identity`에 없다. 직원과 다른 주체다 | `ddl/001`·`002` 확장 · `06` §1 | 진행 |
| **A4** | 원장 카테고리 — 포인트 · 쉐어 | `point_earn`·`point_convert`·`share_accum`이 목표 ENUM에 없다. 전환은 한쪽만 기록돼 분개 합이 0이 아니다 | `04` §16 · `ddl/001`·`003`·`004` | 진행 |
| **A7** | `01` §13 확장 | 25줄이 코드 3,069줄을 대표 중. 파트너·플레이어 컬렉션 24종이 미열거 | `01` §13 전면 개정 | 진행 |
| **A6** | 결함 ID 재배정 | `P2` ≠ `P-02`. 충돌 | 이 문서 §7 확정 후 각 문서 갱신 | 진행 |
| **A9** | 상호 링크 | 세 문서 세트가 서로를 가리키지 않는다 | 각 README | 진행 |
...
A5  파트너 주체            ddl/001 party_type · 002 · 06 §1
A4  포인트 · 쉐어 계정      ddl/001 account_kind · entry_category
                          003 부트스트랩 · 004 posting_rules · 04 §16
A7  01 §13 기준선 정정      컬렉션 24종 · 쓰기 9곳 · category 10종
A6  결함 ID 재배정          P2 → TA-P2
A9  상호 링크 · 매트릭스 갱신
```

**A4의 팁(`avatar_tip`·`dealer_tip`)과 가입 보너스는 이번 범위에서 뺀다** — 아바타 도메인
